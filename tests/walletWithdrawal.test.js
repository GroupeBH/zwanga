...
test("a withdrawal intent survives restart with the same key, amount and recipient", async () => {
  ...
  const load = () =>
    ...
    })("services/walletWithdrawalIntent.ts");
  const api = load();
  const [first, second] = await Promise.all([
    api.prepareWalletWithdrawalIntent("user", 40, "+243891234567"),
    api.prepareWalletWithdrawalIntent("user", 40, "+243891234567"),
  ]);
  assert.equal(first.idempotencyKey, second.idempotencyKey);
  assert.deepEqual(
    await load().prepareWalletWithdrawalIntent("user", 40, "+243891234567"),
    first,
  );
  await assert.rejects(
    api.prepareWalletWithdrawalIntent("user", 41, "+243891234567"),
  );
  await assert.rejects(
    api.prepareWalletWithdrawalIntent("user", 40, "+243899999999"),
  );
  assert.equal(await api.readWalletWithdrawalIntent("other"), null);
  await api.clearWalletWithdrawalIntent({
    ...first,
...
test("corrupted storage blocks new withdrawals instead of losing an uncertain request", async () => {
  const disk = storage();
  disk.data.set("wallet-withdrawal-intent:v1:user", "{broken");
  const api = loader({
    "@react-native-async-storage/async-storage": disk,
    "expo-crypto": { randomUUID },
  })("services/walletWithdrawalIntent.ts");
...
function app(patch = {}) {
  const hooks = hookHarness(),
    ...
    posts = [];
  const state = {
    user: { id: "user", phone: "+243891234567" },
    active: true,
    error: null,
    history: [],
    ...patch,
  ...
  const summary = {
    account: { userId: "user", balance: 100, withdrawableBalance: 60 },
    withdrawal: {
      enabled: true,
      blocked: false,
      currency: "CDF",
  ...
  const load = loader({
    ...
    "@react-native-async-storage/async-storage": disk,
    "expo-crypto": { randomUUID },
    "@/components/ui/DialogProvider": {
      useDialog: () => ({ showDialog: (value) => dialogs.push(value) }),
    },
    "@/store/hooks": { useAppSelector: () => state.user },
    "@/store/selectors": { selectUser: () => state.user },
    "@/features/driver-earnings/payoutModel": {
      normalizePayoutPhone: (value) =>
        /^\+243\d{9}$/.test(value) ? value : null,
    },
    "@/utils/errorHelpers": {
    ...
    "@/store/api/walletApi": {
      ...
      useCheckWalletWithdrawalMutation: () => [
        () => ({
          unwrap: async () => ({ id: "withdrawal", status: "pending" }),
        }),
      ],
    },
  });
  ...
  const render = () =>
    hooks.render(() => useWalletWithdrawal(summary, state.active));
...
test("no spending of loyalty balance, no submission after logout/navigation, and storage failure blocks POST", async () => {
  ...
  value = g.render();
  value.confirm();
  g.disk.setItem = async () => {
    throw new Error("disk full");
  };
  await g.dialogs.at(-1).actions[1].onPress();
  assert.equal(g.posts.length, 0);
  assert.equal(g.render().storageError, true);
});
...
test("API preserves eligibility fields and older backend summaries fail closed", () => {
  ...
  const parsed = endpoints.getMyWallet.transformResponse({
    account: {
      balance: "100",
    ...
    withdrawal: { enabled: true, blocked: true },
  });
  assert.equal(parsed.account.withdrawableBalance, 60);
  assert.equal(parsed.account.reservedWithdrawalBalance, 40);
  assert.equal(parsed.withdrawal.blocked, true);
  const old = endpoints.getMyWallet.transformResponse({
    account: { balance: "100" },
  });
  assert.equal(old.account.withdrawableBalance, 0);
  assert.equal(old.withdrawal, undefined);
});

test('backgrounding during durable storage keeps the same intent without submitting', async () => {
  const f = app(); f.render(); await flush();
  let value = f.render(); value.setTokens('40'); value = f.render(); value.confirm();
  const write = f.disk.setItem;
  f.disk.setItem = async (...args) => {
    await write(...args);
  ...
  value = f.render();
  assert.equal(value.tokens, '40');
  const originalKey = value.activeIntent.idempotencyKey;
  value.confirm(); await f.dialogs.at(-1).actions[1].onPress();
  assert.equal(f.posts.length, 1);
  assert.equal(f.posts[0].idempotencyKey, originalKey);
});
