...
import {
  clearWalletWithdrawalIntent,
  prepareWalletWithdrawalIntent,
  readWalletWithdrawalIntent,
  type WalletWithdrawalIntent,
} from "@/services/walletWithdrawalIntent";
...
export function useWalletWithdrawal(
  ...
  useEffect(() => {
    ...
    setPhone(user?.phone ?? "");
    if (userId)
      void readWalletWithdrawalIntent(userId)
        .then((value) => {
          if (active) {
            setIntent(value);
            setReadyFor(userId);
  ...
  const stillHere = () =>
    current.current.mounted && current.current.userId === userId;
  const present = (value: WalletWithdrawal) =>
    showDialog({
      variant: value.status === "succeeded" ? "success" : "info",
      title:
        value.status === "succeeded" ? "Retrait effectué" : "Suivi du retrait",
  ...
  const clear = async (value: WalletWithdrawalIntent) => {
    ...
    if (stillHere())
      setIntent((old) =>
        old?.idempotencyKey === value.idempotencyKey ? null : old,
      );
  };
  useEffect(() => {
    ...
    if (
      withdrawals.some(
        (value) => value.idempotencyKey === intent.idempotencyKey,
      )
    ) {
      void clear(intent).catch(() => {
  ...
  const submit = async (
    count: number,
    destination: string,
    restored: WalletWithdrawalIntent | null,
  ) => {
    if (
      !userId ||
      !stillHere() ||
    ...
    try {
      try {
        stored =
          restored ??
          (await prepareWalletWithdrawalIntent(userId, count, destination));
      } catch {
        if (stillHere()) setStorageError(true);
      ...
      if (stillHere() && current.current.isActive) {
        setTokens("");
        present(result);
      }
    } catch (error) {
      ...
      if (
        ...
        (error as { data?: { code?: string } })?.data?.code ===
          "WALLET_WITHDRAWAL_NOT_RESERVED"
      ) {
        try {
          await clear(stored);
        } catch {
      ...
      if (stillHere() && current.current.isActive)
        showDialog({
          variant: "info",
          title: "Retrait à vérifier",
          message: getApiErrorMessage(
            error,
            "La réponse est inconnue. Vérifiez cette même demande ; ne créez pas un second retrait.",
          ),
        });
    } finally {
  ...
  const confirm = () => {
    ...
    if (
      ...
      count < 1 ||
      Math.round(count * 100) / 100 !== count ||
      (!activeIntent && count > available)
    ) {
      showDialog({
        variant: "warning",
        title: "Vérifiez le retrait",
        message:
          "Saisissez au moins 1 jeton, sans dépasser le solde retirable, et un numéro Mobile Money valide.",
      });
      return;
    }
    let consumed = false;
    showDialog({
      ...
      actions: [
        ...
        {
          label: activeIntent ? "Vérifier" : "Confirmer",
          variant: "primary",
          onPress: () => {
            if (consumed) return;
            consumed = true;
  ...
  const checkStatus = async (value: WalletWithdrawal) => {
    ...
    setBusy(true);
    try {
      const result = await check(value.id).unwrap();
      if (stillHere() && current.current.isActive) present(result);
    } catch {
      if (stillHere() && current.current.isActive)
        showDialog({
          variant: "info",
          ...
          message:
            "Le retrait n’a pas été relancé. Vérifiez à nouveau plus tard.",
        });
    } finally {
      inFlight.current = false;
      if (current.current.mounted) setBusy(false);
  ...
  return {
    tokens: activeIntent ? String(activeIntent.tokens) : tokens,
    phone: activeIntent?.phone ?? phone,
    setTokens,
    setPhone,
    activeIntent,
    canSubmit,
